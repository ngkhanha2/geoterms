...
public class TDictionary {
    ...
    public TDictionary(String s) {
        try {
            ...
            this.langComparator = new TComparator(this.Info.maSapXep);
    ...
    public void TatKetNoi() {
        try {
            ...
            Logger.getLogger(TDictionary.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    private void write(String s) throws UnsupportedEncodingException, IOException {
        byte[] buffer = s.getBytes("UTF-8");
        this.raf.writeShort(buffer.length);
        this.raf.write(buffer, 0, buffer.length);
    }
    ...
    private int binarySearch(String entry) {
        ...
        while (left <= right) {
            mid = (left + right) >> 1;
            if (this.langComparator.compare(entry, (String) this.model.getElementAt(mid)) <= 0) {
                right = mid - 1;
            } else {
                left = mid + 1;
            }
...
//OVERiDe
    public int insert(String entry, String definition) {
        ...
        try {
            ...
            this.raf.write(bList, 0, index * 4);
            this.raf.writeInt(this.startListFilePointer - this.Info.info.length - 2);
            this.raf.write(bList, index * 4, bList.length - index * 4);

            this.raf.seek(7L);

            this.raf.writeInt(filePointer - this.Info.info.length - 2);
            this.startListFilePointer = filePointer;
    ...
    public void edit(int index, String newEntry, String newDefinition) {
        try {
            if ((index >= this.model.getSize()) || (index < 0)) {
                return;
            }
            changeListFilePointer(index);
            String entry = read();
